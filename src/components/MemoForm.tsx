...
export default function MemoForm({
  ...
  const generateTags = useCallback(async (title: string, content: string) => {
    ...
    try {
      ...
      if (data.tags && Array.isArray(data.tags) && data.tags.length > 0) {
        // 기존 태그와 중복되지 않는 태그만 추가, # 제거
        setFormData(prev => {
          const existingTags = prev.tags
          const cleanedTags = data.tags.map((tag: string) => cleanTag(tag))
          const newTags = cleanedTags.filter(
            (tag: string) => tag && !existingTags.includes(tag)
          )
          return {
            ...prev,
  ...
  const handleGenerateTags = () => {
    ...
    generateTags(formData.title, formData.content)
  }
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                >
                  {isGeneratingTags ? (
                    <>
                      <svg
              ...
              {/* 태그 생성 오류 메시지 */}
              {tagError && (
                <div className="mb-3 text-sm text-red-600">{tagError}</div>
              )}
